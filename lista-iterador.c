...
int removeIt(sIterador *i) {
	...
	if (inicioIt(i))
		i->lista->sentinela->prox = i->it->prox;
	if (fimIt(i))
		i->lista->sentinela->ant = i->it->ant;
	iteraProximo(i);
	free(no->elem);
	free(no);
	i->lista->qtd--;
...
int removeAntIt(sIterador *i) {
	...
	else if (i->it->ant == i->lista->sentinela->prox)
		i->lista->sentinela->prox = i->it;
	free(no->elem);
	free(no);
	i->lista->qtd--;
	return 1;
...
int removeProxIt(sIterador *i) {
	...
	if (fimIt(i))
		i->lista->sentinela->prox = no->prox;
	else if (i->it->prox == i->lista->sentinela->ant)
		i->lista->sentinela->ant = i->it;
	free(no->elem);
	free(no);
	i->lista->qtd--;
	return 1;
